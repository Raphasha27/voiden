...
export const editorApi = {
  showContextMenu: (info: { x: number; y: number; selectedText?: string }) =>
    ipcRenderer.send("show-editor-context-menu", info),
  showCopyContextMenu: (info: {
    x: number;
    y: number;
    selectedText?: string;
...
export const tabApi = {
  ...
  activate: (panelId: string, tabId: string) =>
    ipcRenderer.invoke("tab:activate", panelId, tabId),
  add: (tabId: string, tab: any) => ipcRenderer.invoke("tab:add", tabId, tab),
  registerPanel: (panelId: string, tab: any) =>
    ipcRenderer.invoke("tab:registerPanel", panelId, tab),
  getActiveTab: () => ipcRenderer.invoke("tab:getActiveTab"),
};

export const sidebarApi = {
  getTabs: (sidebarId: "left" | "right") =>
    ipcRenderer.invoke("sidebar:getTabs", sidebarId),
  activateTab: (sidebarId: "left" | "right", tabId: string) =>
    ipcRenderer.invoke("sidebar:activateTab", sidebarId, tabId),
  registerSidebarTab: (
  ...
  setHistoryEnabled: (enabled: boolean) =>
    ipcRenderer.invoke("sidebar:setHistoryEnabled", enabled),
};
...
export const voidenApi = {
  getApyFiles: (directory: string) =>
    ipcRenderer.invoke("voiden-wrapper:getApyFiles", directory),
  getBlockContent: (filePath: string) =>
    ipcRenderer.invoke("voiden-wrapper:getBlockContent", filePath),
};
...
export const envApi = {
  ...
  saveYamlTrees: (
    ...
    ipcRenderer.invoke("env:saveYamlTrees", {
      publicTree,
      privateTree,
      profile,
      projectPath,
...
export const requestApi = {
  ...
  pauseWss: (obj: { wsId: string; reason: string }) =>
    ipcRenderer.send("ws-pause", obj),
  ...
  listenSecure: (
    eventName: string,
    callback: (e: Electron.IpcRendererEvent, data: any) => void,
  ) => {
  ...
  sendGrpcMessage: (grpcId: string, payload: string) =>
    ipcRenderer.invoke("grpc:send", grpcId, payload),
  ...
  connectGraphQLSubscription: (subscriptionId: string) =>
    ipcRenderer.invoke("connect-graphql-subscription", subscriptionId),
  closeGraphQLSubscription: (subscriptionId: string, reason?: string) =>
    ipcRenderer.invoke("close-graphql-subscription", {
      subscriptionId,
      reason,
    }),
};
...
export const scriptApi = {
  executePython: (payload: any) =>
    ipcRenderer.invoke("script:executePython", payload),
  executeNode: (payload: any) =>
    ipcRenderer.invoke("script:executeNode", payload),
};

export const fileLinkApi = {
  exists: (absolutePath: string) =>
    ipcRenderer.invoke("fileLink:exists", absolutePath),
};
...
export const userSettingsApi = {
  ...
  set: (patch: Partial<Settings>): Promise<Settings> =>
    ipcRenderer.invoke("usersettings:set", patch),
...
export const cliApi = {
  ...
  showInstructions: (): Promise<void> =>
    ipcRenderer.invoke("cli:showInstructions"),
};
...
export const variablesApi = {
  ...
  readMerged: (envKey?: string) =>
    ipcRenderer.invoke("variables:readMerged", envKey),
  ...
  get: (key: string, envKey?: string) =>
    ipcRenderer.invoke("variables:get", key, envKey),
  set: (key: string, value: any, envKey?: string) =>
    ipcRenderer.invoke("variables:set", key, value, envKey),
  writeVariables: (content: string | Record<string, any>, envKey?: string) =>
    ipcRenderer.invoke("variables:writeVariables", content, envKey),
  mergeVariables: (vars: Record<string, any>, envKey?: string) =>
    ipcRenderer.invoke("variables:mergeVariables", vars, envKey),
  deleteKey: (key: string, envKey?: string) =>
    ipcRenderer.invoke("variables:deleteKey", key, envKey),
};
...
export const mainWindow = {
  ...
  closeAndDeleteState: () =>
    ipcRenderer.invoke("mainwindow:closeAndDeleteState"),
